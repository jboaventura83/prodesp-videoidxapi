...
namespace VideoIdxApp.API.Controllers
{
    ...
    public class VideosController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetVideo(string id)
        {
            ...
            if(videoFromApi != null)
            {
               var videoToReturn = new VideosForListDto{
                   ...
                   ThumbnailId = "",
                   Duration = videoFromApi.durationInSeconds,
                   Created = videoFromApi.created.ToString("dd-MM-yyyy"),
        ...
        [HttpGet("{id}/thumbnail/{thumbnailId}", Name = "GetVideoThumbnail")]
        public async Task<IActionResult> GetVideoThumbnail(string id, string thumbnailId)
        {
            var client = new HttpClient();
            var queryString = HttpUtility.ParseQueryString(string.Empty);
            ...
            queryString["accessToken"] = accessToken;
            var uri = "https://api.videoindexer.ai/"+ _config.GetSection("AppSettings:LocationApi").Value + 
                "/Accounts/"+ _config.GetSection("AppSettings:AccountIdApi").Value  +"/Videos/" + id +"/Thumbnails/"+ thumbnailId +"?" + queryString;

            var response = await client.GetAsync(uri);

            return Ok("{\"base64image\":\"" + await response.Content.ReadAsStringAsync() + "\"}");
        }
